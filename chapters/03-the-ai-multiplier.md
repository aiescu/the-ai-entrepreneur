...
Three weeks after your first fleet drill, you brace for the same slog on your second build: the spec you'll agonize over, the same corrections typed into the same agents, another evening wiring a test harness from scratch. You open the repo from the first build to remind yourself how you did it — and stop. The spec is still there, and most of it is the shape of the new problem too. The little file you wrote the night an agent kept botching your pull-request format is still there, and it makes the new agents get it right on the first try. The eval scaffold is still there. You copy three of them over, change maybe a fifth of each, and by the time you'd normally have finished writing the spec, the second build is already running behind a green check.

You did not work faster. You started from a different line. That night you try to figure out why it was cheap — not cheaper the way a good night's sleep makes tomorrow cheaper, but cheap in a way that felt like it shouldn't have been allowed, like you'd been handed the answers to a test you were sure you never took.
