...
Three weeks after the night he ran his first fleet, Adrian starts his second real build. It is 9:40 on a Thursday at the same kitchen table where forty applications a month used to get written — dishes done, second bedtime story read — and he braces for the same slog the first build was: the spec he agonized over for three evenings, the same corrections typed into the same agents, another late hour wiring a test harness from scratch. He opens the repo from the first build just to remind himself how he did it — and stops. The spec is still there, and most of it is the shape of the new problem too. The little file he wrote the night an agent kept botching his pull-request format is still there, and it makes the new agents get it right on the first try. The eval scaffold is still there. He copies three of them over, changes maybe a fifth of each, and by the time he would normally have finished writing the spec, the second build is already running behind a green check.

He did not work faster. He started from a different line. That night he tries to figure out why it was cheap — not cheaper the way a good night's sleep makes tomorrow cheaper, but cheap in a way that feels like it should not have been allowed, like being handed the answers to a test he was sure he never took. His first instinct, trained by months of silence from the funnel, is suspicion: cheap must mean wrong. He rereads the diff hunting for the mistake that would explain the speed. There is no mistake. The check is green because the check is real.

Here is the question this chapter exists to answer: what if the value of AI was never the hours it saved him on the build in front of him, but the assets it let him manufacture on the way — assets that make every build after this one start from a line the last one drew?
