...
It is nine on a Tuesday and Adrian has two jobs in front of him, both his, both due — due in the way things get due when the only project manager is the severance calendar, and the calendar says March. One is a migration: move the build off the deprecated payments client onto the new one, mechanical and tedious and touching a dozen files. The other is a feature he actually wants to build, a small export endpoint with its own tests. The greedy thought arrives on its own: why not run both at the same time, two agents, one evening, and go to bed with two things shipped instead of one? He tried exactly that last week — opened two agents in the same folder, gave them a job each, and went to empty the dishwasher. Forty minutes later he came back to a working tree where both agents had edited the same config file, each convinced it was right, the diff a knot he spent an hour untangling — an hour that ended past midnight with less shipped than one agent would have managed alone.

So tonight he does the human thing, which is to pick. He is not slow and he is not disorganized. He is obeying a rule so deep he has never once said it out loud — that he is one person, at one keyboard, and one person can only be in one place at a time. But the knot last week was not caused by the number of his hands. It was caused by two workers sharing one desk.

Here is the question this chapter exists to answer: what if the thing stopping Adrian from running five jobs at once was never that he has one head, but that he kept making the workers share a room?

## The false belief: "I can only be in one place at a time"

Name the rule that ran under Adrian's whole evening, because it runs under yours too, even if you've never put it in words: *I can only be in one place at a time.* One body, one keyboard, one train of thought that snaps the moment you split it. It feels less like a belief than a law of physics, because for your entire career it was one — the work happened where your hands were. The belief survived agents by mutating: *fine, an agent can type without me, but I still have to watch it.* Two agents need two sets of eyes you don't have, and you're back to one at a time. The hidden premise — the whole load-bearing wall — is that parallel work requires your *simultaneous presence*. Strip it out and the fear collapses, because you already know it's false: you don't hover over two database queries or two CI jobs. You launch them, they run without you, and you look when the results land. The reason you can't do that with two agents isn't a fact about your attention. It's a fact about how you set the work up.
